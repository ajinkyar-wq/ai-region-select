...
export async function segmentImage(
  ...
  try {
    ...
    const peoplePath = maskToPath(peopleMask, maskWidth, maskHeight, scaleX, scaleY, 1.5);
    if (peoplePath) {
      regions.push({
        id: 'people-' + Date.now(),
        type: 'people',
    ...
    // Foreground mask (category 5: others/objects)
    const foregroundMask = extractMaskFromCategory(result.categoryMask, 5, maskWidth, maskHeight);
    const foregroundPath = maskToPath(foregroundMask, maskWidth, maskHeight, scaleX, scaleY, 1.5);
    if (foregroundPath) {
      regions.push({
      ...
      // Create a fallback foreground region
      regions.push({
        id: 'foreground-' + Date.now(),
        type: 'foreground',
        pathData: createOrganicShape(canvasWidth / 2, canvasHeight * 0.75, canvasWidth * 0.3, canvasHeight * 0.15, 8, 0.2),
        visible: true,
        selected: false,
      });
    }
    ...
    const backgroundPath = maskToPath(backgroundMask, maskWidth, maskHeight, scaleX, scaleY, 2);
    if (backgroundPath) {
      regions.push({
